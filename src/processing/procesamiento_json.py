...
def aplicar_filtros_mensajes_json(mensajes_crudos_df: pd.DataFrame, filtros_mensajes: list[EstrategiaFiltro]) -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    mensajes_descartados = {} # diccionario, clave: nombre del filtro, valor: dataframe con los mensajes descartados por ese filtro
    for estrategia in filtros_mensajes:
        nombre_filtro = estrategia.nombre()
        # dataframe con los mensajes descartados por ese filtro
        mensajes_descartados_df = mensajes_limpios_df[mensajes_limpios_df["content"].apply(estrategia.aplicar)]
        # se guarda en el diccionario con clave el nombre del filtro y valor el dataframe con los mensajes descartados por ese filtro
        ...
        # se actualiza el dataframe de mensajes limpios aplicando el filtro, quedando solo los mensajes que no fueron descartados por ese filtro
