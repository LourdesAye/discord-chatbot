...
from typing import List, Tuple, Optional # para la anotación de tipos y así mejorar la legibilidad del código y la detección de errores
...
def get_ruta_completa_archivo(nombre_archivo: str, directorio_inicial: Optional[Path] = None) -> Path:
    """
    Busca `nombre_archivo` recorriendo hacia arriba desde `directorio_inicial`.
    Devuelve la Path del archivo si lo encuentra, si no lanza FileNotFoundError.
    """
    if directorio_inicial is None:
        try: # protección si __file__ no está definido
            directorio_inicial = Path(__file__).resolve().parent  # agarra el directorio (parent) que contiene al archivo actual(__file__)
        except NameError:
            directorio_inicial = Path.cwd()
    ...
    while True:
        ...
        if posible_ruta_env.exists() and posible_ruta_env.is_file(): # si existe la ruta del archivo .env 
...
def cargar_ruta_directorio_archivo(nombre_archivo: str = ".env")-> Path: # flineame: str significa que es cadena de texto y que su valor por defecto es ".env"
...
class BuscadorArchivos:
    ...
    # --- obtiene solo rutas de los JSONs ---
    def obtener_rutas_json(self, directorio_base: Path = None) -> List[Ruta]:
        ...
        return self._buscar_archivos(
            directorio_base,
            self._criterios_busqueda["json"]
        )

    # --- obtiene solo rutas de las imágenes ---
    def obtener_rutas_imagenes(self, directorio_base: Path = None) -> List[Ruta]:
        directorio_base = directorio_base or JSON_DIR_ABS

        if not directorio_base.exists():
            raise FileNotFoundError(f"Directorio base no existe: {directorio_base}")

        return self._buscar_directorios(
            directorio_base,
            self._criterios_busqueda["imagenes"]
        )
    ...
    def _buscar_archivos(self, directorio: Path, criterios: dict) -> List[Ruta]:
        encontrados: List[Ruta] = []
        criterio_busqueda = criterios["nombre_archivo"]
        profundidad_maxima = criterios.get("profundidad_maxima",999)
        for archivo in directorio.rglob(criterio_busqueda): # busca en el directorio actual y todos sus subdirectorios los archivos llamados chat.json. 
    ...
    def _buscar_directorios(self, directorio: Path, criterios: dict) -> List[Ruta]:
        encontrados: List[Ruta] = []
        criterio_busqueda = criterios["nombre_directorio"]
        profundidad_maxima = criterios.get("profundidad_maxima",999)
        for dir_imagen in directorio.rglob(criterio_busqueda):# busca en el directorio actual y todos sus subdirectorios los directorios llamados images. 
