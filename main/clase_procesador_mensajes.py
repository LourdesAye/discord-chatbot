...
from utilidades.utilidades_logs import setup_logger,guardar_pregunta_y_respuestas_en_log,guardar_respuestas_sin_pregunta
from utilidades.utilidades_conversiones import convertir_a_datetime,tiempo_transcurrido
    ...
    def _procesar_respuesta_docente(self, mensaje: Mensaje):
        if self.preguntas_abiertas: # si hay elementos en la lista preguntas_abiertas 
            ...
            if len(self.preguntas_cerradas) >= 2: # se valida que la lista de preguntas_cerradas tenga al menos dos preguntas 
                ...
                logger_msj.debug(f" 🔴 MENSAJE SIN PREGUNTAS ABIERTAS O CERRDAS : '{mensaje.contenido}'. DOCENTE:  '{mensaje.autor}' ")
